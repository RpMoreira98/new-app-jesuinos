...
export class PostgreSQLDB {
  ...
  private async init() {
    ...
    if (!hasDbUrl) {
      console.warn(
        "[POSTGRES] DATABASE_URL não está configurada. Ativando Modo Fallback Recipiente em Memória para visualização imediata!",
      );
    ...
    try {
      ...
      console.log(
        "[POSTGRES] Conectado com sucesso! Inicializando tabelas de dados no PostgreSQL (Supabase)...",
      );
      ...
      const configCountRes = await pool.query(
        "SELECT COUNT(*) as count FROM business_config",
      );
      ...
      const rodrigoHash = bcrypt.hashSync(
        process.env.RODRIGO_PASSWORD || "Rodrigo@SecurePass2026!",
        10,
      );
      ...
      const adminCheck = await pool.query(
        ...
        ["admin@jesuinosbarbearia.com.br"],
      );
      ...
      // Upsert rodrigo user
      const rodrigoCheck = await pool.query(
        "SELECT * FROM users WHERE email = $1",
        ["rodrigopontes126@gmail.com"],
      );
      ...
      console.warn(
        "[POSTGRES] Conexão com PostgreSQL falhou ou está instável. Erro:",
        error,
      );
      console.log(
