  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!clientName || !clientEmail || !clientPhone || !date || !time) {
        return res.status(400).json({ error: 'Preencha todos os campos obrigatórios (nome, email, telefone, data e horário).' });
      }
    ...
    return res.status(500).json({ error: 'Erro interno no servidor de agendamentos: ' + error.message });
  }
};
